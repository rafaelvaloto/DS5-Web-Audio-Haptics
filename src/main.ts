...
export function createAudioHapticsController(options: {
    ...
    const applySettings = (mode = settings.rumbleMode) => {
        ...
        options.api.initializeAudio?.(Math.min(1, Math.max(0, settings.audioVolume / 100)), Math.min(2.0, Math.max(1.0, settings.gain)));
    };
  ...
  public setAudioHapticsStateListener(listener: ((enabled: boolean) => void) | null): void {
    ...
    public setAudioHapticsSettings(settings: Partial<AudioHapticsSettings>): void {
        this.audioHapticsSettings = {
            ...
            gain: Math.min(2.0, Math.max(1.0, settings.gain ?? this.audioHapticsSettings.gain)),
...
function bindNativeApi(module: NativeModule): NativeApi {
    ...
    return {
        ...
        initializeAudio: maybe("GCH_InitializeAudio", null, ["number", "number"]) as NativeApi["initializeAudio"],
