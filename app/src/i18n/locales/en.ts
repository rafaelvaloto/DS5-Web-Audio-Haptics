import type { TranslationDictionary } from "../types.ts";

export const en: TranslationDictionary = {
	installPage: {
		title: "Install DS5 Web Audio Haptics Extension",
		brand: "DS5 Web Audio Haptics",
		github: "GitHub",
		videoTutorial: "Video Tutorial",
		videoTutorialTriggers: "Triggers Tutorial",
		eyebrow: "Recommended experience",
		heroTitle: "Bring audio haptics immersion to your web games.",
		heroDescription:
			"Real-time Bluetooth audio and haptics, adaptive triggers, and a customizable lightbar without installing drivers or emulators. The extension runs in a side panel on the same game tab, keeping the experience discreet without getting in the way of gameplay.",
		installExtension: "Install Extension",
		downloadGamepadSocket: "Download Gamepad Socket",
		heroCallout:
			"For browser games, the extension can now use <strong>Activate in Tab</strong> to emulate a keyboard or gamepad without extra installs. Gamepad Socket is only optional for cases that still need a virtual gamepad outside that flow.",
		whyTitle: "Why the extension?",
		whyDescription:
			"It makes audio capture, browser input injection, and setup more consistent than the plain hosted page flow.",
		pillBrowserSupport: "Better browser-game support",
		pillAudioCapture: "Smoother tab audio capture",
		pillInstallPath: "Dedicated install path",
		pillOnboarding: "Focused user onboarding",
		stepOneTitle: "1. Install the extension",
		stepOneItemOne: "Install the browser extension from the Chrome Web Store.",
		stepOneItemTwo: "Open your browser game or supported page.",
		stepOneItemThree:
			"Use <strong>Activate in Tab</strong> to enable browser keyboard or gamepad emulation directly in the current tab.",
		stepTwoTitle: "2. Connect your controller",
		stepTwoItemOne: "Pair the DualSense with Windows over Bluetooth.",
		stepTwoItemTwo: "Open the extension and connect the controller.",
		stepTwoItemThree: "Allow audio/tab capture when the browser asks for permission.",
		stepTwoItemFour: "Make sure audio sharing is enabled so sound can play through the controller.",
		stepThreeTitle: "3. Optional extras",
		stepThreeItemOne: "Enable audio + haptics from the extension when you start playing.",
		stepThreeItemTwo:
			"Use <strong>Gamepad Socket</strong> only if a specific game still needs virtual gamepad routing outside the in-tab flow.",
		stepThreeItemThree: "Install <strong>ViGEmBus</strong> only when Gamepad Socket requires it.",
		partnersLabel: "Partners",
		partnersTitle: "Where to play",
		partnersDescription:
			"Play on partner platforms where browser games fit naturally with this extension.",
		playOnItchIo: "Play on itch.io",
		playOnCrazyGames: "Play on partner platform",
		playOnPoki: "Play on Poki",
		whereToPlayTitle: "Browser platforms",
		whereToPlayDescription:
			"Choose a platform to start playing. More curated links and partnerships can be added here over time.",
		footerCopyright: "© 2026 Valoto Games. All rights reserved.",
		footerNote:
			"Native Windows games with direct DualSense Bluetooth support do not need Gamepad Socket, and browser games can use Activate in Tab without extra installs.",
		footerDisclaimer:
			"PlayStation, DualSense, itch.io, CrazyGames, and Poki are trademarks of their respective owners. This is an independent project and is not officially affiliated with those brands or platforms.",
	},
	options: {
		title: "DualSense Pairing",
		description: "Authorize Chrome to read your DualSense controller. After pairing, close this page and use the side panel.",
		loading: "Loading controller engine...",
		connect: "Connect DualSense",
		none: "No authorized controller found.",
		ready: "Controller engine ready.",
		authorizedSuccess: "Controller authorized successfully.",
	},
	header: {
		btnShowLogs: "Show Logs",
		btnCloseLogs: "Close Logs",
		title: "DS5 Web Audio Haptics Bluetooth",
		subtitle: "High-fidelity audio and haptics via Bluetooth for Next-Gen DualSense controllers using WebHID and WebAssembly",
		btnLoadWasm: "1) Load WASM",
		btnConnectHid: "2) Connect DualSense (WebHID)",
		btnStartLoop: "3) Play",
		btnStopLoop: "4) Stop",
		btnAudioHaptics: "🎵 Audio Haptics (Off)",
		btnAudioHapticsOn: "🎵 Audio Haptics (On)",
		btnPip: "🪟 Picture-in-Picture",
		btnClearLogs: "Clear Logs",
		language: "Language",
	},
	status: {
		wasm: "WASM",
		notLoaded: "Not Loaded",
		ready: "Ready",
		error: "Error",
		device: "Device",
		none: "None",
		connected: "Connected",
		gyro: "Gyroscope",
		touch: "Touchpad",
		audioHaptics: "Audio Haptics",
		active: "Active",
		inactive: "Inactive",
		battery: "Battery",
		rate: "Rate:",
	},
	cards: {
		commands: {
			title: "🎮 Action & Trigger Controls",
			subtitle: "Interactive testing for Adaptive Triggers, Lightbar LED and Vibration",
		},
		analogs: {
			title: "🕹️ Analogs & Triggers (Sticks & Triggers)",
			subtitle: "Continuous analog values and L2/R2 trigger thresholds",
		},
		buttons: {
			title: "🔘 Digital Buttons & System",
			subtitle: "Face buttons, D-Pad, system buttons, and back paddles (DualSense Edge)",
		},
		motion: {
			title: "🧭 Gyroscope & Motion (IMU 6-Axis)",
			subtitle: "Sensors enabled via GCH_EnableGyroscopeValues",
		},
		touchpad: {
			title: "👆 Touchpad (Capacitive Touch)",
			subtitle: "Enabled via GCH_EnableTouch",
		},
		logs: {
			title: "📜 Real-Time Diagnostic & Event Log",
			subtitle: "Real-time readings from 148-byte buffer and executed commands",
		},
	},
	controls: {
		changeTrigger: "Switch between triggers R3 + D-Pad (Up=1, Right=2, Down=3, Left=Reset)",
		createTrigger: "Create Trigger",
		connectionFlow: "Connection Flow",
		audioSettingsTitle: "Audio Controls",
		headsetOnly: "Audio Only Mode",
		modeAudioOnly: "Audio Only Mode",
		gain: "Gain",
		volume: "Volume",
		sharedPiPTitle: "Enable Audio Haptics Bluetooth",
		triggerEffectsTitle: "Adaptive Trigger Effects",
		selectTriggerEffect: "Choose Trigger Effect",
		targetTrigger: "Target Trigger",
		applyTrigger: "Apply Trigger Effect",
		resetTriggers: "Reset Triggers",
		lightbarTitle: "Lightbar LED Color",
		selectColor: "Select Color",
		applyColor: "Apply LED Color",
		vibrationTitle: "Rumble & Vibration Test",
		testSoft: "Test Soft Rumble",
		testHeavy: "Test Heavy Rumble",
		testBoth: "Test Both Motors",
		stopVibration: "Stop Vibration",
		stopAll: "🛑 Stop All (Motors, Triggers & Lights)",
		leftTrigger: "Left (L2)",
		rightTrigger: "Right (R2)",
		bothTriggers: "Both (L2 + R2)",
		effectNone: "None / Off",
		effectGameCube: "GameCube (R2)",
		effectGallop: "Gallop (L2)",
		effectMachine: "Machine Gun (R2)",
		effectFeedback: "Rigid Feedback (R2)",
		effectBow: "Bow Tension (R2)",
		effectWeapon: "Weapon Semi-Auto (R2)",
		effectAutoGun: "Automatic Gun Buzz (R2)",
		inputServerHint:
			"For native Windows games that do not support the DualSense via Bluetooth, follow the steps below:<br><br>Install the <a\n" +
			'\t\t\t\thref="https://github.com/nefarius/ViGEmBus/releases/"\n' +
			'\t\t\t\ttarget="_blank"\n' +
			'\t\t\t\trel="noopener noreferrer"\n' +
			"\t\t\t>ViGEmBus_1.22.0_x64_x86_arm64.exe</a\n" +
			"\t\t\t> driver to emulate a virtual gamepad.",
		inputServerRepository: "To connect the controller to a virtual gamepad, run the socket:",
		browserGamepadTitle: "🎮 Browser Gamepad",
		browserGamepadHint: "For browser games, enable this option.",
		browserGamepadConnect: "Enable for Current Tab",
		browserGamepadDisconnect: "Disable",
		browserGamepadConnecting: "Connecting...",
		browserGamepadConnected: "Connected",
		browserGamepadDisconnected: "Disconnected",
		browserKeyboardTitle: "⌨️ Keyboard",
		browserKeyboardHint: "Use for games without gamepad support.",
		browserKeyboardHintOne: "Use for games without gamepad support.",
		browserKeyboardConnect: "Enable for Current Tab",
		browserKeyboardDisconnect: "Disable",
		browserKeyboardConnecting: "Connecting...",
		browserKeyboardConnected: "Connected",
		browserKeyboardDisconnected: "Disconnected",
		browserKeyboardMap: "Map Keys",
	},
	keyboardMapping: {
		title: "Keyboard mapping",
		description: "Click a control, press the desired key, and the mapping will be saved automatically.",
		defaultPreset: "Default FPS layout: WASD on the left stick, Q/E on L1/R1, and L2/R2 unassigned.",
		mouseLockedHint: "The right stick stays fixed to mouse movement for camera control.",
		mouseButtonsHint: "To map mouse input, click a control and then use left click, right click, middle click, or the wheel.",
		reset: "Restore default layout",
		unassigned: "Unassigned",
		pressAnyKey: "Press any key...",
		leftAnalogUp: "Left analog - up",
		leftAnalogDown: "Left analog - down",
		leftAnalogLeft: "Left analog - left",
		leftAnalogRight: "Left analog - right",
		dpadUp: "D-Pad - up",
		dpadDown: "D-Pad - down",
		dpadLeft: "D-Pad - left",
		dpadRight: "D-Pad - right",
		cross: "Cross (X)",
		circle: "Circle (O)",
		square: "Square",
		triangle: "Triangle",
		leftShoulder: "L1",
		rightShoulder: "R1",
		leftTrigger: "L2",
		rightTrigger: "R2",
		start: "Options / Start",
		share: "Create / Share",
	},
	commands: {
		cross: "Cross (X)",
		circle: "Circle (O)",
		square: "Square ([ ])",
		triangle: "Triangle (/\\)",
		l1: "L1",
		r1: "R1",
		up: "Up",
		down: "Down",
		left: "Left",
		right: "Right",
	},
	analogs: {
		l1: "L1",
		l2: "L2 (Trigger)",
		r1: "R1",
		r2: "R2 (Trigger)",
		threshold: "Threshold",
		deadzone: "DeadZone",
		active: "Active",
		leftStick: "Left Analog (L-Stick)",
		rightStick: "Right Analog (R-Stick)",
		l3Click: "L3 (Click)",
		r3Click: "R3 (Click)",
		up: "Up",
		down: "Down",
		left: "Left",
		right: "Right",
	},
	buttons: {
		cross: "Cross (X)",
		circle: "Circle (O)",
		square: "Square ([ ])",
		triangle: "Triangle (/\\)",
		dpadUp: "D-Pad UP",
		dpadDown: "D-Pad DOWN",
		dpadLeft: "D-Pad LEFT",
		dpadRight: "D-Pad RIGHT",
		ps: "PS Button",
		share: "Create/Share",
		options: "Options/Start",
		touchClick: "Touch Click",
		mute: "Mute Button",
		phone: "Headset Connected",
		fn1: "Fn 1",
		fn2: "Fn 2",
		paddleL: "Paddle L",
		paddleR: "Paddle R",
	},
	motion: {
		gyroscope: "Gyroscope (Rotational Velocity)",
		accelerometer: "Accelerometer (Linear Acceleration)",
		gravity: "Gravity (Gravity Vector)",
		tilt: "Tilt (Pitch, Roll, Yaw)",
	},
	touchpad: {
		status: "Touch Status",
		touching: "Touching",
		notTouching: "Not Touching",
		touchId: "Touch ID",
		fingers: "Fingers",
		direction: "Direction",
		position: "Position (X, Y)",
		relative: "Relative (X, Y)",
		radius: "Radius (X, Y)",
	},
	logs: {
		wasmLoading: "Waiting for WASM module initialization...",
		wasmLoaded: "✅ WASM GamepadCoreHost loaded successfully.",
		wasmFailed: "❌ Error loading WASM: {{error}}",
		webHidPrompt: "Opening WebHID device selection dialog...",
		webHidConnected: "✅ Authorized device(s): {{name}}",
		webHidNotFound: "❌ Device not found.",
		loopStarted: "▶️ Reading and telemetry loop started.",
		loopStopped: "⏹️ Loop paused.",
		gyroEnabled: "🧭 Gyroscope enabled successfully.",
		touchEnabled: "👆 Touchpad enabled successfully.",
		triggerApplied: "⚙️ Trigger effect applied.",
		rumbleApplied: "⚙️ Rumble applied.",
		allStopped: "🛑 All stopped.",
		notConnected: "❌ Disconnected",
	},
	triggers: {
		pageTitle: "DualSense - Adaptive Triggers",
		composeAndSave: "🛠️ Compose and Save Effect",
		gameLabel: "Game",
		gamePlaceholder: "Ex: Returnal",
		effectNameLabel: "Effect Name",
		effectNamePlaceholder: "Ex: Weapon Name",
		effectModeLabel: "Effect Mode (Byte 0)",
		modeFeedback: "0x21 - Feedback (Resistance)",
		modeBow: "0x22 - Bow",
		modeGalloping: "0x23 - Galloping",
		modeWeapon: "0x25 - Weapon",
		modeAutoGun: "0x26 - Automatic Gun",
		modeMachine: "0x27 - Machine",
		handTriggerLabel: "Hand Trigger",
		payloadHexLabel: "Payload HEX",
		testTriggerBtn: "Test Trigger",
		saveProfileBtn: "Save Trigger to Profile",
		savedProfilesTitle: "Saved Profiles",
		clearAllBtn: "Clear All",
	},
};
