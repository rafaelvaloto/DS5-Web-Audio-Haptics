...
class DualSenseSocketBridge {
  ...
  send(state: FInputContext): void {
    ...
    if (this.sentCount === 1 || this.sentCount % 100 === 0)
      console.debug(`[DualSense Socket] State sent #${this.sentCount}`);
  }
  close(): void {
    this.disconnect();
  }
...
// Trigger Effect Payloads
const TRIGGER_GALLOPING = new Uint8Array([
  0x23, 0x82, 0x00, 0xf7, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00,
]);
const TRIGGER_MACHINE = new Uint8Array([
...
const TRIGGER_GAMECUBE = new Uint8Array([
  0x25, 0x90, 0x02, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
]);
...
type NativeApi = {
  ...
  initializeAudio?: (volume: number, gain: number) => void;
...
export function createAudioHapticsController(options: {
  ...
  const settings: AudioHapticsSettings = {
    ...
    audioVolume: Math.min(
      100,
      Math.max(0, options.settings?.audioVolume ?? 100),
    ),
    gain: Math.min(2.0, Math.max(1.0, options.settings?.gain ?? 1)),
    rumbleMode: options.settings?.rumbleMode === 0xff ? 0xff : 0xfc,
    rumbleReduce: Math.min(
      15,
      Math.max(0, options.settings?.rumbleReduce ?? 0),
    ),
  };

  const applySettings = (mode = settings.rumbleMode) => {
    for (const id of options.deviceIds) {
      options.api.dualsenseSettings?.(
        id,
        0,
        1,
        settings.bIsSpeaker,
        ...
        mode,
        settings.rumbleReduce,
        0,
      );
      options.api.updateOutput?.(id);
    }
    options.api.initializeAudio?.(
      Math.min(1, Math.max(0, settings.audioVolume / 100)),
      Math.min(2.0, Math.max(1.0, settings.gain)),
    );
  };
  ...
  const submit = (
    ...
    new Float32Array(heap.buffer, heap.byteOffset + bufferPtr, data.length).set(
      data,
    );
  ...
  const disable = async () => {
    ...
    if (processor && "port" in processor)
      (processor as AudioWorkletNode).port.close();
    source?.disconnect();
    mute?.disconnect();
  ...
  const enable = async () => {
    ...
    if (nextStream.getAudioTracks().length === 0) {
      ...
      throw new Error(
        "Nenhuma faixa de áudio disponível. Marque 'Compartilhar áudio'.",
      );
    }
    try {
      ...
      if (
        nextContext.audioWorklet &&
        typeof nextContext.audioWorklet.addModule === "function"
      ) {
        await nextContext.audioWorklet.addModule("audio-worklet.js");
        const node = new AudioWorkletNode(
          nextContext,
          "audio-haptics-worklet-processor",
        );
      ...
      nextStream.getTracks().forEach((track) =>
        track.addEventListener("ended", () => {
          if (enabled) disable().catch(console.error);
        }),
      );
      stream = nextStream;
      context = nextContext;
...
export class GamepadClientApplication {
  ...
  private readonly dsExtensionBridge = new DualSenseSocketBridge();
  ...
  private readonly managesDiscovery: boolean;
  private isRunning = false;
  private inputStateListener:
    ((deviceId: number, state: FInputContext) => void) | null = null;
  ...
  private constructor(
    ...
    this.managesDiscovery = managesDiscovery;
   ...
   */
  public static fromNativeRuntime(
    module: NativeModule,
    api: NativeApi,
    deviceIds: Set<number>,
  ): GamepadClientApplication {
  ...
  static async create(
    typeId = 0,
    requestAccess = true,
  ): Promise<GamepadClientApplication> {
    if (typeof navigator === "undefined" || !("hid" in navigator)) {
      throw new Error(
        "Este loop usa WebHID e precisa rodar no navegador (com interação do usuário).",
      );
    }

    if (requestAccess) await requestSonyWebHidAccess();
    ...
    const app = new GamepadClientApplication(
      module,
      api,
      platformCleanup,
      registryCleanup,
  ...
  run(): void {
    ...
    this.isRunning = true;
    let discoveryHandle: ReturnType<typeof setInterval> | null = null;
    if (this.managesDiscovery) {
    ...
    const inputHandle = setInterval(() => {
      ...
      for (const deviceId of ids) {
        ...
        this.inputStateListener?.(deviceId, state);
      }
    }, FRAME_MS);
  ...
  public setInputStateListener(
    listener: ((deviceId: number, state: FInputContext) => void) | null,
  ): void {
    this.inputStateListener = listener;
  }
  ...
  public connectInputServer(url: string): void {
    this.dsExtensionBridge.connect(url);
  }
  public disconnectInputServer(): void {
    this.dsExtensionBridge.disconnect();
  }
  public isInputServerConnected(): boolean {
    return this.dsExtensionBridge.isConnected();
  }
  public setInputServerStatusListener(
    listener: Parameters<DualSenseSocketBridge["setStatusListener"]>[0],
  ): void {
    this.dsExtensionBridge.setStatusListener(listener);
  }
  ...
  public setAudioHapticsStateListener(
    listener: ((enabled: boolean) => void) | null,
  ): void {
  ...
  public setAudioHapticsSettings(
    settings: Partial<AudioHapticsSettings>,
  ): void {
    this.audioHapticsSettings = {
      ...
      audioVolume: Math.min(
        ...
        Math.max(
          0,
          Math.round(
            settings.audioVolume ?? this.audioHapticsSettings.audioVolume,
          ),
        ),
      ),
      gain: Math.min(
        2.0,
        Math.max(1.0, settings.gain ?? this.audioHapticsSettings.gain),
      ),
      ...
      rumbleReduce: Math.min(
        ...
        Math.max(
          0,
          Math.round(
            settings.rumbleReduce ?? this.audioHapticsSettings.rumbleReduce,
          ),
        ),
      ),
    };
    ...
    if (this.isAudioHapticsEnabled) {
      for (const deviceId of this.deviceIds) {
        this.api.dualsenseSettings?.(
          ...
          this.audioHapticsSettings.rumbleMode,
  ...
  private applyAudioHapticsSettings(
    deviceId: number,
    rumbleMode = this.audioHapticsSettings.rumbleMode,
  ): void {
    this.api.dualsenseSettings?.(
      deviceId,
      0,
  ...
  public initializeAudio(volume = 1.0, gain = 1.0): void {
    this.api.initializeAudio?.(volume, gain);
  }

  private initializeAudioFromSettings(): void {
    const gain = Math.min(2.0, Math.max(1.0, this.audioHapticsSettings.gain));
    const volume = Math.min(
      1,
      Math.max(0, this.audioHapticsSettings.audioVolume / 100),
    );
    ...
    if (ctx.audioWorklet && typeof ctx.audioWorklet.addModule === "function") {
      ...
      workletNode.port.onmessage = (event: MessageEvent) => {
        ...
        const floatView = new Float32Array(
          heap.buffer,
          heap.byteOffset + this.audioBufferPtr,
          totalFloats,
        );
        floatView.set(audioData);

        this.api.audioSubmitSamples(
          this.audioBufferPtr,
          frameCount,
          numChannels,
          ctx.sampleRate,
      ...
      this.audioHapticsStateListener?.(true);
      this.initializeAudioFromSettings();
      for (const deviceId of this.deviceIds) {
  ...
  public async disableAudioHaptics(): Promise<void> {
    ...
    this.audioHapticsStateListener?.(false);
...
export async function startGamepadClientLoop(
  typeId = 0,
): Promise<GamepadClientApplication> {
...
function bindNativeApi(module: NativeModule): NativeApi {
  ...
  return {
    setLogCallback: maybe("GCH_SetLogCallback", null, [
      "number",
    ]) as NativeApi["setLogCallback"],
    discoverDevices: must("GCH_DiscoverDevices", null, [
      "number",
    ]) as NativeApi["discoverDevices"],
    updateInput: must("GCH_UpdateInput", null, [
      "number",
      "number",
    ]) as NativeApi["updateInput"],
    getInputState: must("GCH_GetInputState", null, [
      "number",
      "number",
    ]) as NativeApi["getInputState"],
    enableGyroscopeValues: maybe("GCH_EnableGyroscopeValues", null, [
      "number",
      "number",
    ]) as NativeApi["enableGyroscopeValues"],
    enableTouch: maybe("GCH_EnableTouch", null, [
      "number",
      "number",
    ]) as NativeApi["enableTouch"],
    resetGyroOrientation: maybe("GCH_ResetGyroOrientation", null, [
      "number",
    ]) as NativeApi["resetGyroOrientation"],
    batteryLevelDevice: maybe("GCH_BatteryLevelDevice", "number", [
      "number",
    ]) as NativeApi["batteryLevelDevice"],
    setVibration: maybe("GCH_SetVibration", null, [
      "number",
      "number",
      "number",
    ]) as NativeApi["setVibration"],
    lightbar: maybe("GCH_Lightbar", null, [
      "number",
      "number",
      "number",
      "number",
    ]) as NativeApi["lightbar"],
    resetLights: maybe("GCH_ResetLights", null, [
      "number",
    ]) as NativeApi["resetLights"],
    updateOutput: maybe("GCH_UpdateOutput", null, [
      "number",
    ]) as NativeApi["updateOutput"],
    customTrigger: maybe("GCH_CustomTrigger", "number", [
      "number",
      "number",
      "number",
      "number",
    ]) as NativeApi["customTrigger"],
    stopTrigger: maybe("GCH_StopTrigger", null, [
      "number",
      "number",
    ]) as NativeApi["stopTrigger"],
    shutdown: maybe("GCH_Shutdown", null, []) as NativeApi["shutdown"],
    audioSubmitSamples: maybe("GCH_AudioSubmitSamples", "number", [
      "number",
      "number",
      "number",
      "number",
    ]) as NativeApi["audioSubmitSamples"],
    initializeAudio: maybe("GCH_InitializeAudio", null, [
      "number",
      "number",
    ]) as NativeApi["initializeAudio"],
    getProcessAudioHaptics: (maybe("GCH_GetProcessAudioHaptics", "number", [
      "number",
    ]) ||
      maybe("GCH_ProcessAudioHaptics", "number", [
        "number",
      ])) as NativeApi["getProcessAudioHaptics"],
    processAudioHaptics: (maybe("GCH_GetProcessAudioHaptics", "number", [
      "number",
    ]) ||
      maybe("GCH_ProcessAudioHaptics", "number", [
        "number",
      ])) as NativeApi["processAudioHaptics"],
    dualsenseSettings: maybe("GCH_DualSenseSettings", null, [
      "number",
      "number",
      "number",
      "number",
...
async function loadGamepadCoreHostFactory(): Promise<
  (moduleArg?: Record<string, unknown>) => Promise<unknown>
> {
  const globalFactory = (globalThis as { InitGamepadCoreHost?: unknown })
    .InitGamepadCoreHost;
  if (typeof globalFactory === "function") {
    return globalFactory as (
  ...
  if (typeof candidate !== "function") {
    throw new Error(
      "GamepadCoreHost.js inválido: export esperado é função de inicialização.",
    );
  }
